...
%lineColor=[.45,.25,.15];
lineColor=colors*.5;
boxColor=[colors(1)*1.2 colors(2)*1.3 colors(3)*1.4 .6];
...
set(dah,'defaultlinelinewidth',1)
%% Correlation
hold(cah,'on');
for groupi=1:groups
	if strcmpi(symb,'Num')
		for i=1:n
			...
			text(data1((groupi-1)*n+i),data2((groupi-1)*n+i),'*','parent',cah,'fontsize',markersize,'color',colors(floor((groupi-1)/s(2))+1,:), 'HorizontalAlignment','Center', 'VerticalAlignment','Middle');
		end
	else
		if s(3)==1
			marker = symb(1);
...
if ischar(axesLimits)
	if strcmpi(axesLimits,'Auto')
		...
		if strcmpi(symb,'Num')
			...
			ph = plot(cah, [mindata maxdata], [mindata maxdata], 'o');
		end
		axesLimits = axis(cah); 
...
plot(cah,axesLimits(1:2), polyval(polyCoefs,axesLimits(1:2)),'Color',lineColor,'LineWidth',lineWidth);
h = plot(cah,axesLimits(1:2),axesLimits(1:2),':'); set(h,'color',[0.6 0.6 0.6],'LineWidth',1);
...
tposX=axesLimits(1)+0.01*(axesLimits(2)-axesLimits(1));
tposY=axesLimits(1)+0.9*(axesLimits(2)-axesLimits(1));
t=text(tposX,tposY,corrtext,'parent',cah,'FontSize',labelFontSize,'BackGroundColor',boxColor);
%Check if the text overlaps the line
tpos=get(t,'extent'); %[left,bottom,width,height]
data2mean=mean(data2);
padding=(axesLimits(2)-axesLimits(1))/10;
% if data2mean<(tpos(2)+tpos(4))
%     %Adjust position
%     %Above line
%     if (axesLimits(2)-(data2mean+.5*tpos(4)))>padding
%         set(t,'Position',[tposX+padding*.5,data2mean+.5*tpos(4)+padding]);
%     else
%         %Below line
%         set(t,'Position',[tposX+padding*.5,data2mean-.5*tpos(4)-padding]);
%     end
% end
xlabel(cah,labelx,'FontSize',labelFontSize); ylabel(cah,labely,'FontSize',labelFontSize);
	...
	if strcmpi(symb,'Num')
		for i=1:n
			if xdatamode==1
                ...
                text(mean([d1(i),d2(i)]), dif(i), 'o', 'parent',dah,'fontsize',markersize,'color',colors(floor((groupi-1)/s(2))+1,:));
			else
...
axis(dah,'square')
xlabel(dah,labelm,'FontSize',labelFontSize); ylabel(dah,labeld,'FontSize',labelFontSize);
...
cr = 1.96*st;
meanperc=100*cr/mean((data2(mask)+data1(mask))/2);
...
plot(a(1:2),mn+[0 0],'Color',lineColor,'LineWidth',lineWidth)
% plot(a(1:2),mn+st*[1 1],'k')
% plot(a(1:2),mn-st*[1 1],'k')
plot(a(1:2),mn+cr*[1 1],'Color',lineColor,'LineStyle','--','LineWidth',lineWidth)
plot(a(1:2),mn-cr*[1 1],'Color',lineColor,'LineStyle','--','LineWidth',lineWidth)
a = axis(dah);
%offsetx=.5; offsety=-.5;
offsetx=0; offsety=0;
if fixylim
    ...
    ylim auto
	t1=text(a(2),mn+cr,{'+1.96SD',num2str(mn+cr,2)},'HorizontalAlignment','left','VerticalAlignment','middle','fontsize',fontsize,'BackGroundColor',boxColor);
    tpos=get(t1,'extent');
    set(t1,'Position',[a(2)-tpos(3)*.5,tpos(2)+tpos(4)*.5]);
	t2=text(a(2),mn,{num2str(mn,2),['p=' num2str(p,2)]},'HorizontalAlignment','left','VerticalAlignment','middle','fontsize',fontsize,'BackGroundColor',boxColor);
    tpos=get(t2,'extent');
    set(t2,'Position',[a(2)-tpos(3)*.5,tpos(2)+tpos(4)*.5]);
	t3=text(a(2),mn-cr,{num2str(mn-cr,2),'-1.96SD'},'HorizontalAlignment','left','VerticalAlignment','middle','fontsize',fontsize,'BackGroundColor',boxColor);
    tpos=get(t3,'extent');
    set(t3,'Position',[a(2)-tpos(3)*.5,tpos(2)+tpos(4)*.5]);
...
% 	end
% end
% text(a(2),a(4)-offsety,BAtext,'interpreter','tex','HorizontalAlignment','right','VerticalAlignment','top','FontSize',24);
...
set(cah,'XTickLabel',get(cah,'XTickLabel'),'FontSize',labelFontSize)
set(dah,'XTickLabel',get(dah,'XTickLabel'),'FontSize',labelFontSize)
if nargout>2
	sstruct = struct('N',N,...
		'CR', cr,...
        'MeanPerc',meanperc,...
